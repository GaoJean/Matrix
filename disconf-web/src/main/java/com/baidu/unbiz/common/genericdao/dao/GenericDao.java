 ...
 * 虚拟的父类
 */
public abstract class GenericDao<KEY extends Serializable, ENTITY extends BaseObject<KEY>>
                                                                                           implements
                                                                                           BaseDao<KEY, ENTITY> {
    ...
    public List<ENTITY> find(List<Match> matches, List<Order> order, int offset, int limit) {
        // FIXME
        Query operate = queryGenerator.getSelectQuery(matches,
            (order == null) ? null : order.toArray(new Order[order.size()]));
     ...
     * @return
     */
    public <T extends Object, O extends Object> List<T> findBySQL(String sql, List<O> params,
                                                                  RowMapper<T> mapper) {
    ...
    public ENTITY create(ENTITY entity, InsertOption option) {
        ...
        boolean autoGeneratedKey = (Number.class.isAssignableFrom(orMapping.getKeyClass()) && (key == null || (Long
            .parseLong(key.toString()) == 0)));

        // 获取insert语句
        @SuppressWarnings("unchecked")
        Query query = queryGenerator.getCreateQuery(autoGeneratedKey, false, option, entity);
        final String sql = query.getSql();
        System.out.println("数据库sql为：" + sql);
    ...
    public int[] insert(List<ENTITY> entities, InsertOption option) {
        ...
        for (int index = 0, length = entities.size(); index < length; index++) {
            int updated = createAndFetchUpdateRow(entities.get(index), query.getSql(), query
                .getParams().subList(i, i += paramPerEntity));
